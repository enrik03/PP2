...
# first graph fun.
def graph(step, ke, pe, etotal,dif_cof):
    ...
    diff_coff = plt.plot(step, dif_cof, label = 'Diffusion coeficient')
    
    plt.xlabel('Steps')
    plt.ylabel('Energy')
    plt.title('Energy & Diff. coefficient vs steps Graph')
    plt.grid()
...
def data(
    ...
    while nw < 2:
        ...
        nw += 1
        k = 0
        while k < 8:  # this loop selects a k-column
            while i < ev:  # this loop selects the lines, it begins in 104 (it depends with the file log.lammps)
                ...
                if k == 0:
                    ...
                    dif_cof.append(float(mylines[i-1].split()[k]))
                    #print(dif_cof)
    ...
    return graph(temp, kee, pee, total, dif_cof)
